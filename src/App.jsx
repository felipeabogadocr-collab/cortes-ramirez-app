...
const TOOLS_ASISTENTE = [
  ...
  {
    name: "crear_usuario",
    description: "Crea un nuevo usuario con acceso al panel del despacho (un miembro del equipo).",
    input_schema: {
      type: "object",
      properties: {
        nombre: { type: "string" },
        rol: { type: "string", description: "Administrador, Abogado o Asistente" },
      },
      required: ["nombre"],
    },
  },
...
async function ejecutarHerramienta(nombreHerramienta, input) {
  ...
  if (nombreHerramienta === "crear_usuario") {
    const raw = await storageGet("usuarios-despacho", true);
    const usuarios = raw ? JSON.parse(raw) : [];
    const nuevo = { id: uid(), nombre: input.nombre, rol: input.rol || "Asistente" };
    await storageSet("usuarios-despacho", JSON.stringify([...usuarios, nuevo]), true);
    return { mensaje: `Usuario "${input.nombre}" creado con acceso al panel, con rol de ${nuevo.rol}.` };
  }
      ...
      <div ref={contenedorRef} style={{ minHeight: 260, maxHeight: 520, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginBottom: 12, paddingRight: 4 }}>
        {mensajes.length === 0 && (
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 12.5, color: COLORS.muted }}>
            Pregúntame qué priorizar hoy, pídeme ideas de negocio o de contenido, mándame una foto, un PDF o un Word, o dime cosas como "crea un cliente llamado...", "registra un pago de...", "crea un caso para...", "crea un usuario para..." o "actualiza el estado de vigilancia de...". Recuerdo todo lo que hablemos aquí.
          </p>
        )}
        {mensajes.map((m, i) => (
          <div
...
function useUsuariosDespacho() {
  ...
  return { usuarios, cargado, agregar, actualizar };
}
...
function LoginGate({ onIngresar, onCancelar }) {
  const { usuarios, cargado, agregar } = useUsuariosDespacho();
  const { oscuro, alternar } = useTema();
  const [pantalla, setPantalla] = useState("login"); // login | bootstrap
  const [nombre, setNombre] = useState("");
  ...
  useEffect(() => {
    if (cargado && usuarios.length === 0) setPantalla("bootstrap");
  }, [cargado, usuarios.length]);

  useEffect(() => {
    (async () => {
      const recordado = await storageGet("ultimo-usuario-login", false);
      if (recordado) setUsuarioLogin(recordado);
    })();
  }, []);

  const cambiarCampoLogin = (campo, valor) => {
    if (campo === "usuario") setUsuarioLogin(valor);
    else setContrasena(valor);
    if (validado) setValidado(false);
    if (error) setError("");
  };

  const crearAdministrador = async () => {
    if (!nombre.trim() || !usuarioLogin.trim() || !contrasena.trim()) return;
    const nuevo = await agregar(nombre.trim(), usuarioLogin.trim(), contrasena, "Administrador", permisosPorDefecto("Administrador"));
    await storageSet("ultimo-usuario-login", usuarioLogin.trim(), false);
    onIngresar(nuevo);
  };

  const validarCredenciales = () => {
    const encontrado = usuarios.find((u) => u.usuario?.toLowerCase() === usuarioLogin.trim().toLowerCase());
    ...
    setError("");
    setUsuarioValidado(encontrado);
    setValidado(true);
  };

  const confirmarIngreso = async () => {
    if (!validado || !usuarioValidado) return;
    await storageSet("ultimo-usuario-login", usuarioLogin.trim(), false);
    onIngresar(usuarioValidado);
  };
    ...
    >
      ...
      <Card style={{ maxWidth: 420, width: "100%", textAlign: "center", position: "relative" }}>
        ...
        {pantalla === "bootstrap" && (
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <Field label="Usuario">
                <input className="drx-input" style={inputStyle} value={usuarioLogin} onChange={(e) => setUsuarioLogin(e.target.value)} placeholder="Ej: felipe.cortes" />
              </Field>
            ...
            <button
              ...
              disabled={!nombre.trim() || !usuarioLogin.trim() || !contrasena.trim()}
            >
          ...
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <Field label="Usuario">
                <input
                  ...
                  style={inputStyle}
                  value={usuarioLogin}
                  onChange={(e) => cambiarCampoLogin("usuario", e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && validarCredenciales()}
                  autoFocus
                />
              </Field>
              <Field label="Contraseña">
                <CampoContrasena valor={contrasena} onChange={(v) => cambiarCampoLogin("clave", v)} onEnter={validarCredenciales} autoFocus={!!usuarioLogin} />
              </Field>
            </div>
            {error && <p style={{ color: "#B42318", fontSize: 12.5, marginTop: 10, fontFamily: "Inter, sans-serif" }}>{error}</p>}
            {validado && (
              <p style={{ color: "#2F5D3A", fontSize: 12.5, marginTop: 10, fontFamily: "Inter, sans-serif", fontWeight: 600 }}>✓ Identidad validada</p>
            )}
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <button
                ...
                disabled={!usuarioLogin.trim() || !contrasena.trim()}
              >
                Validar
              </button>
              <button className="drx-btn-primary" style={{ ...buttonPrimary, flex: 1 }} onClick={confirmarIngreso} disabled={!validado}>
                Ingresar
              </button>
            </div>
        ...
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 10.5, color: COLORS.muted, marginTop: 20, lineHeight: 1.5 }}>
          Esta ya es la interfaz real de usuario y contraseña para que la pruebes. El cifrado real de la contraseña se
          activa al publicar la versión final.
        </p>
      </Card>
    </div>
  );
...
function UsuariosPermisosTab({ usuarioActualId }) {
  const { usuarios, agregar, actualizar } = useUsuariosDespacho();
  const [mostrarForm, setMostrarForm] = useState(false);
  const [nombre, setNombre] = useState("");
  const [usuarioLogin, setUsuarioLogin] = useState("");
  const [contrasena, setContrasena] = useState("");
  ...
  const crear = async () => {
    if (!nombre.trim() || !usuarioLogin.trim() || !contrasena.trim()) return;
    await agregar(nombre.trim(), usuarioLogin.trim(), contrasena, rol, permisosPorDefecto(rol));
    setNombre("");
    setUsuarioLogin("");
    setContrasena("");
    setRol("Asistente");
      ...
      {mostrarForm && (
        <Card style={{ marginBottom: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <Field label="Usuario">
              <input className="drx-input" style={inputStyle} value={usuarioLogin} onChange={(e) => setUsuarioLogin(e.target.value)} />
            </Field>
            <Field label="Contraseña">
          ...
          <button className="drx-btn-primary" style={{ ...buttonPrimary, marginTop: 12 }} onClick={crear} disabled={!nombre.trim() || !usuarioLogin.trim() || !contrasena.trim()}>
            Crear usuario
          </button>
        </Card>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const idGuardado = await storageGet("sesion-usuario-id", false);
      if (idGuardado) {
        const raw = await storageGet("usuarios-despacho", true);
        const usuarios = raw ? JSON.parse(raw) : [];
        const encontrado = usuarios.find((u) => u.id === idGuardado);
        if (encontrado) setUsuarioActual(encontrado);
      }
  ...
  const iniciarSesion = async (usuario) => {
    ...
    await storageSet("sesion-usuario-id", usuario.id, false);
  };
