...
// Contrato de prestación de servicios para un cliente nuevo — se genera
// desde la Calculadora de precios, antes de que el cliente exista en
// Nomos como tal (por eso recibe los datos sueltos del formulario, no un
// objeto de cliente ya guardado). El valor y las cuotas vienen ya
// calculados por la calculadora, para que el contrato y lo que se le
// mostró al cliente en pantalla nunca queden desincronizados.
...
// tener que crear el cliente primero solo para sacar el contrato.
function PanelCalculadoraPrecios({ datosResponsable }) {
  const [abierto, setAbierto] = useState(false);
  ...
  return (
    <Card style={{ marginBottom: 20 }}>
      ...
      >
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 14, fontWeight: 700, color: COLORS.ink, margin: 0, display: "flex", alignItems: "center", gap: 6 }}>
          <Icono tipo="calculadora" size={14} /> Calculadora de precios
        </p>
        <span style={{ fontFamily: "Inter, sans-serif", fontSize: 12.5, color: COLORS.muted }}>{abierto ? "Ocultar ▲" : "Abrir ▼"}</span>
      </button>
      {!abierto && (
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 12, color: COLORS.muted, marginTop: 6 }}>
          Calcula cuánto te queda libre y cuánto sería cada cuota antes de proponerle un valor al cliente, y descarga el contrato de servicios ya listo.
        </p>
      )}
...
export default function ContabilidadTab({ usuarioActual, clienteInicialPago, onClienteInicialPagoConsumido }) {
  ...
  return (
    <div>
      ...
      <PanelCalculadoraPrecios datosResponsable={datosResponsable} />
